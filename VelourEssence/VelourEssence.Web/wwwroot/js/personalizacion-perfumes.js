...
class PersonalizacionPerfumes {
    ...
    async agregarAlCarrito() {
        ...
        try {
            ...
            // Preparar los datos del producto personalizado
            const selecciones = Array.from(this.currentSelections.entries()).map(([criterioId, opcionId]) => ({
                idCriterio: criterioId,
                idOpcion: opcionId
            }));

            const productoPersonalizado = {
                idProductoBase: this.baseProductoId,
                cantidad: 1,
                selecciones: selecciones
            };

            // Llamar al controlador de carrito
            const response = await fetch('/Carrito/AgregarProductoPersonalizado', {
                method: 'POST',
                headers: {
            ...
            // Por ahora simular éxito ya que no tenemos la implementación completa
            btn.innerHTML = '<i class="fas fa-check"></i> ¡Simulado!';
            btn.classList.add('btn-success');
